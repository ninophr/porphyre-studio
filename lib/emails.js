...
<div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;color:#333">
  <div style="background:#121010;padding:32px;border-radius:12px">
    <h1 style="color:#c4956a;font-size:24px;margin:0 0 8px">Merci ${h(brief.nom)} !</h1>
  ...
  <div style="padding:24px 0">
    ...
    <table style="width:100%;border-collapse:collapse">
      <tr><td style="padding:8px 0;color:#666">Offre</td><td style="padding:8px 0;font-weight:600">${h(offreName)} — ${h(prix)}</td></tr>
      <tr><td style="padding:8px 0;color:#666">Bien</td><td style="padding:8px 0">${h(brief.bien)}</td></tr>
      <tr><td style="padding:8px 0;color:#666">Localisation</td><td style="padding:8px 0">${h(brief.localisation)}</td></tr>
    </table>
...
<div style="font-family:monospace;max-width:700px;margin:0 auto;color:#333">
  <div style="background:#121010;padding:24px;border-radius:12px">
    <h1 style="color:#c4956a;font-size:20px;margin:0">Nouvelle commande ${h(offreName)}</h1>
  </div>
  <h2 style="margin-top:20px">Client</h2>
  <table style="width:100%;border-collapse:collapse;font-size:14px">
    <tr><td style="padding:4px 8px;color:#666;width:140px">Nom</td><td>${h(brief.nom)}</td></tr>
    <tr><td style="padding:4px 8px;color:#666">Email</td><td><a href="mailto:${h(brief.email)}">${h(brief.email)}</a></td></tr>
    <tr><td style="padding:4px 8px;color:#666">Téléphone</td><td>${h(brief.telephone || "—")}</td></tr>
    <tr><td style="padding:4px 8px;color:#666">Bien</td><td>${h(brief.bien)}</td></tr>
    <tr><td style="padding:4px 8px;color:#666">Localisation</td><td>${h(brief.localisation)}</td></tr>
    <tr><td style="padding:4px 8px;color:#666">Type</td><td>${h(brief.type_bien || "—")}</td></tr>
    <tr><td style="padding:4px 8px;color:#666">Offre</td><td><strong>${h(offreName)} — ${h(prix)}</strong></td></tr>
    ${brief.lien_airbnb ? `<tr><td style="padding:4px 8px;color:#666">Airbnb</td><td><a href="${h(brief.lien_airbnb)}">${h(brief.lien_airbnb)}</a></td></tr>` : ""}
    ${brief.domaine ? `<tr><td style="padding:4px 8px;color:#666">Domaine</td><td>${h(brief.domaine)}</td></tr>` : ""}
  </table>
  ${brief.description ? `<h2>Description</h2><p style="font-size:14px;white-space:pre-wrap">${h(brief.description)}</p>` : ""}
...
<div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;color:#333">
  <div style="background:#121010;padding:32px;border-radius:12px">
    ...
    <p style="color:#fff;opacity:.7;margin:0">${h(brief.bien)} — ${h(brief.localisation)}</p>
  </div>
  <div style="padding:24px 0">
    <p style="color:#555">Bonjour ${h(brief.nom)},</p>
...
<div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;color:#333">
  <div style="background:#121010;padding:32px;border-radius:12px">
    <h1 style="color:#c4956a;font-size:24px;margin:0 0 8px">Bonjour ${h(brief.nom)},</h1>
    <p style="color:#fff;opacity:.7;margin:0">Il manque quelques informations pour créer votre site.</p>
  </div>
  <div style="padding:24px 0">
    <p style="color:#555">Pour que je puisse démarrer la création de votre site <strong>${h(brief.bien)}</strong>, j'ai besoin de :</p>
    <ul style="color:#555;line-height:1.8">
