...
const C = {
  bg: "#f5f1e8", // page cream
  bgAlt: "#ebe5d8", // gradient stop
  card: "#fdfcf9", // off-white card (matches showcase cards on site)
  dark: "#1a1715", // footer / dark buttons / nav
  text: "#2b2622", // headings
  body: "#4a4540", // body copy (matches site body)
  taupe: "#7d7770", // secondary / muted
...
function wordmark() {
  // Text-only wordmark for maximum email client compatibility
  // (SVG/PNG logos fail in Outlook/Gmail without remote hosting + alt-text hacks)
  return `
<table role="presentation" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <td style="vertical-align:middle;font-family:${FONT};font-size:15px;font-weight:800;color:${C.text};letter-spacing:-0.01em">
      Porphyre<span style="color:${C.accent};margin:0 2px">.</span>Studio
    </td>
  </tr>
</table>`;
}
...
<tr><td style="padding:0 8px 24px">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${C.bg};background-image:linear-gradient(135deg,#f5f1e8 0%,#ebe5d8 30%,#ede8dd 60%,#f5f1e8 100%);border-radius:32px;border:1px solid rgba(255,255,255,0.9);box-shadow:0 40px 80px -20px rgba(30,25,20,0.15),inset 0 1px 0 rgba(255,255,255,0.8)">
    <tr><td style="padding:44px 42px 46px">
      ${eyebrow ? `<div style="margin-bottom:20px">${pill(eyebrow)}</div>` : ""}
      <h1 style="margin:0;font-family:${FONT};font-size:32px;line-height:1.08;font-weight:800;color:${C.text};letter-spacing:-0.028em">
        ${title}
      </h1>
      ${subtitle ? `<p style="margin:16px 0 0;font-family:${FONT};font-size:15px;color:${C.body};font-weight:500;line-height:1.6">${subtitle}</p>` : ""}
    </td></tr>
  </table>
</td></tr>`;
}
...
<tr><td style="padding:0 8px 16px">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${C.card};border-radius:24px;border:1px solid ${C.border};box-shadow:0 25px 60px -15px rgba(30,25,20,0.12)">
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:6px auto 0">
  <tr><td style="background:${C.dark};border-radius:999px;box-shadow:0 12px 32px -8px rgba(30,25,20,0.4)">
    <a href="${href}" style="display:inline-block;padding:16px 32px;font-family:${FONT};font-size:15px;font-weight:600;color:${C.white};text-decoration:none;letter-spacing:-0.005em">
...
<tr><td style="padding:28px 8px 0">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${C.dark};border-radius:24px;box-shadow:0 12px 32px rgba(30,25,20,0.15)">
    <tr><td style="padding:28px 32px">
      <p style="margin:0 0 10px;font-family:${FONT};font-size:14px;color:rgba(255,255,255,0.78);line-height:1.6">
      ...
      <p style="margin:0;font-family:${FONT};font-size:11px;color:rgba(255,255,255,0.4);letter-spacing:0.02em">
        Porphyre Studio — Sites web pour locations saisonnières
      </p>
    </td></tr>
  </table>
</td></tr>`;
